...
import { findContato, createContato, updateContato, deleteContato } from "../../../src/services/contato.service";
...
describe('Contato Service', () => {
    ...
    describe('sucess', ()=>{
        it('findContato', async() => {
            ...
            await expect(findContato({id})).resolves.toBeDefined()
        });
        ...
        it('updateContato', async() => {
            ...
            await expect(updateContato({id}, {nome: chance.name()})).resolves.toBeDefined()
        });
        
        it('deleteContato', async() => {
            model.delContact = jest.fn().mockResolvedValue([1])
            await expect(deleteContato({id})).resolves.toBeDefined()
        });
    });
    ...
    describe('error', ()=>{
        ...
        it('deleteContato - ID inexistente', async() => {
            ...
            await expect(deleteContato({id: chance.integer()})).resolves.toBe(0)
        });
    });
